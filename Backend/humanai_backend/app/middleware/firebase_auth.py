...
async def get_current_user(
    ...
    try:
        ...
        if settings.APP_ENV == "development":
            try:
                import jwt
                decoded = jwt.decode(token, settings.APP_ENCRYPTION_KEY, algorithms=["HS256"])
            except Exception:
    ...
    tenant_id = decoded.get("tenant_id", "default-tenant")
    employee_id = decoded.get("employee_id")

    if not role or role not in VALID_ROLES:
        raise HTTPException(status_code=403, detail="Rôle non assigné ou invalide")

    # Look up the user in the database to get their database UUID (which is a real UUID)
    # to avoid Postgres UUID casting exceptions in the RLS policies.
    ...
    try:
        ...
        print(f"Error looking up user for RLS: {e}")

    await set_rls_context(db, user_id_str, role, dept_id, tenant_id)
