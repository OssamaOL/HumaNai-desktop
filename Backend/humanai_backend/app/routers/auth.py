...
async def signup(payload: SignupRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        try:
            ...
            # Fallback en mode développement si Firebase Admin échoue (ex: clé révoquée)
            if settings.APP_ENV == "development":
                fb_uid = f"fb_mock_{uuid.uuid4().hex[:12]}"
            else:
                raise e

        await db.execute(insert(User).values(
            id=uuid.uuid4(), firebase_uid=fb_uid,
            email=payload.email, display_name=payload.display_name,
            role=UserRole(payload.role),
        ))
        await db.commit()
        return {"message": "Compte créé avec succès", "uid": fb_uid}
    except Exception as e:
...
async def login(payload: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    # In development mode, skip Firebase and use local JWT directly
    if settings.APP_ENV == "development":
        ...
        if user:
            ...
            return {"idToken": mock_token, "refreshToken": "mock_refresh_token", "expiresIn": 86400}
