from sqlalchemy import Column, String, Text, ForeignKey, Enum as SAEnum, DateTime, Boolean, JSON
...
class HrAlert(Base):
    ...
    severity = Column(SAEnum(AlertSeverity, name="alert_severity_type"), nullable=False)
...
class AiInteraction(Base):
    ...
    data_scope_requested = Column(JSON, nullable=True)
...
class AiSecurityEvent(Base):
    ...
    correlation_id = Column(UUID(as_uuid=True), nullable=True)
    event_type = Column(SAEnum(SecurityEventType, name="security_event_type"), nullable=False)
    severity = Column(SAEnum(AlertSeverity, name="alert_severity_type"), nullable=False)
