...
async def get_redis() -> aioredis.Redis:
    ...
    if settings.APP_ENV == "testing":
        ...
        if _redis_client is None or not isinstance(_redis_client, AsyncMock):
            _redis_client = AsyncMock()
            # Mock commonly used methods to return appropriate values
            _redis_client.get = AsyncMock(return_value=None)
            _redis_client.set = AsyncMock(return_value=True)
            _redis_client.setex = AsyncMock(return_value=True)
            _redis_client.delete = AsyncMock(return_value=True)
            _redis_client.keys = AsyncMock(return_value=[])
            _redis_client.ping = AsyncMock(return_value=True)
            _redis_client.incr = AsyncMock(return_value=1)
            _redis_client.expire = AsyncMock(return_value=True)
